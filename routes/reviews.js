...
router.get('/:bookId', async (req, res) => {
  try {
    ...
    res.json({ reviews, avg_rating: stats.avg ? parseFloat(stats.avg.toFixed(1)) : 0, total_reviews: stats.count });
...
router.post('/:bookId', auth, async (req, res) => {
  try {
    ...
    await db.runAsync('INSERT INTO reviews (user_id, book_id, rating, review_text) VALUES (?,?,?,?) ON CONFLICT(user_id, book_id) DO UPDATE SET rating=excluded.rating, review_text=excluded.review_text', [req.user.id, req.params.bookId, parseInt(rating), review_text || null]);
