...
router.post('/', auth, upload.single('payment_screenshot'), async (req, res) => {
  try {
    ...
    const book = await db.getAsync('SELECT id FROM books WHERE id = ? AND is_active = 1', [book_id]);
    ...
    if (pending) return res.status(409).json({ error: 'A payment is already under review.' });
    ...
    res.json({ message: 'Order submitted. Payment under verification.', order_id: result.lastID });
...
router.get('/my-library', auth, async (req, res) => {
  try {
    const books = await db.allAsync(`
      ...
      ORDER BY rp.last_read DESC, o.created_at DESC
    `, [req.user.id]);
    res.json({ books });
  } catch (err) { res.status(500).json({ error: err.message }); }
});
