...
router.get('/', async (req, res) => {
  try {
    ...
    const params = [];
    if (search) { sql += ' AND (title LIKE ? OR description LIKE ? OR category LIKE ?)'; params.push(`%${search}%`, `%${search}%`, `%${search}%`); }
    ...
    const booksWithRatings = await Promise.all(books.map(async b => {
      ...
      return { ...b, avg_rating: r && r.avg ? parseFloat(parseFloat(r.avg).toFixed(1)) : 0, review_count: r ? parseInt(r.count) : 0 };
    }));
...
router.get('/:id', async (req, res) => {
  try {
    ...
    res.json({ book: { ...book, avg_rating: rating && rating.avg ? parseFloat(parseFloat(rating.avg).toFixed(1)) : 0, review_count: rating ? parseInt(rating.count) : 0, reviews } });
  } catch (err) { res.status(500).json({ error: err.message }); }
});
...
router.get('/:id/pdf', auth, async (req, res) => {
  try {
    ...
    if (!book || !book.pdf_file) return res.status(404).json({ error: 'PDF not available yet.' });
  ...
  try {
    ...
    await db.runAsync(`
      ...
      ON CONFLICT(user_id, book_id) DO UPDATE SET
        current_page=excluded.current_page, total_pages=excluded.total_pages, last_read=CURRENT_TIMESTAMP
